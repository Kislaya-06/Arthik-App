...
### 🚀 Added
- **Native Biometric App Lock:** Added native device screen lock and biometric security (Fingerprint, Face Unlock, and device PIN/Pattern fallback) via `expo-local-authentication`. Automatically secures the app on launch and background minimization, preventing sensitive financial data from appearing in the recent apps task switcher.
